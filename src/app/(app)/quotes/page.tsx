      ...
      <div className="bg-white dark:bg-neutral-900 rounded-xl border border-neutral-200 dark:border-neutral-700 overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-neutral-50 dark:divide-neutral-800">
            {(filtered?.length ?? 0) === 0 && (
              <tr><td colSpan={5} className="px-4 py-10 text-center text-neutral-400 dark:text-neutral-500">No quotes found.</td></tr>
            )}
            {filtered?.map(doc => (
              <tr key={doc.id} className="hover:bg-neutral-50 dark:hover:bg-neutral-800 transition-colors">
                <td className="px-4 py-3">
                  <Link href={`/quotes/${doc.id}`} className="font-medium hover:underline">{doc.number ?? <span className="text-neutral-400 dark:text-neutral-500">Draft</span>}</Link>
                </td>
